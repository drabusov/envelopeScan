...
#	Ksc=1.2e-7 # 1.5e-6     # space charge perveance
	...
	# TUNES
	aux = np.linspace(18.55,18.95,10)
	tunes=[(x,y) for x in aux for y in aux]

	w = open("tune_scan_{}.csv".format(Ksc),"w")
	w.write("qx0,qy0,qx,qy,fIni,fCorr,aIni,aCorr\n")
	for qx0,qy0 in tunes: 
...
#			Dx0[j]=b.get_twiss().Dx

		sVar = np.linspace(0,sCold[-1],2*len(sCold))
		betaX0,betaY0 = np.interp(x=sVar, xp=sCold,fp=betx0,period = sCold[-1]/6.0), np.interp(x=sVar, xp=sCold,fp=bety0,period = sCold[-1]/6.0)
		...
		for j, b in enumerate(BeamLine):
			...
			betxWarm[j]=b.get_twiss().betx
			betyWarm[j]=b.get_twiss().bety
			#Dx0[j]=b.get_twiss().Dx
		...
		matchBareTunes(qx0,qy0,skipMatching=True)
	
		varsDict = findElements("quads.str")
		nQuads = {"KD":83,"KF":83,"KW1":1,"KW2":1}
		quadDict = {"KD":"k1nl_S00QD1D","KF":"k1nl_S00QD1F","KW1":"k1nl_S52QD11","KW2":"k1nl_S52QD12"}

		fIni,fCorr,aIni,aCorr = correction(qx0,qy0,varsDict,sWarm,betaX0,betaY0,sVar)
		arr = [qx0,qy0,line_para["qx"],line_para["qy"]]+[fIni,fCorr,aIni,aCorr]
		w.write("{},{},{},{},{},{},{},{}\n".format(*arr))
